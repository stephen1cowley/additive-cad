...
class ExperimentConfig:
    ...
    Attributes:
        llm_name (str): The name of the language model. Should be a valid huggingface model name, e.g., "huggyllama/llama-7b".
        device (Literal['cpu', 'cuda']): The computation device to use. Must be either 'cpu' or 'cuda'.
        context_prompt (str): The template prompt string used to create prompts with context. Requires {context} and {question} formatters.
            Example: `"{context} Using only the references listed above, answer the following question: Question: {question}. Answer:"`.
        no_context_prompt (str): The template prompt string used when no context is provided. Requires {question} formatter.
